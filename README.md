...
Если при генерации иллюстраций возникает ошибка 500:

1. **Проверьте логи сервера** - в терминале, где запущен `npm run dev`, будут детальные сообщения об ошибке
2. **Проверьте Hugging Face API ключ** - убедитесь, что ключ добавлен в `.env.local` и сервер перезапущен
3. **Проверьте модель** - модель `stabilityai/stable-diffusion-xl-base-1.0` может быть недоступна. Попробуйте другую модель:
   ```env
   HUGGING_FACE_MODEL=runwayml/stable-diffusion-v1-5
   ```
4. **Модель загружается** - при первом запросе модель может загружаться 10-30 секунд. Попробуйте еще раз через некоторое время
5. **Проверьте доступность API** - убедитесь, что ваш API ключ имеет права чтения и модель доступна на Hugging Face
